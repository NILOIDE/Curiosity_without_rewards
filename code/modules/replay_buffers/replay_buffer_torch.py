...
class DynamicsReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        ...
        for i in idxes:
            ...
            obses_t.append(torch.from_numpy(obs_t).type(dtype=torch.float32))
            actions.append(torch.from_numpy(action).type(dtype=torch.float32))
            obses_tp1.append(torch.from_numpy(obs_tp1).type(dtype=torch.float32))
            dones.append(torch.tensor(done, dtype=torch.int8))
        return torch.stack(obses_t), torch.stack(actions), torch.stack(obses_tp1), torch.stack(dones)
